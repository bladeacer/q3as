...
- Every repo path in the docs was an inert code span, so nothing was clickable
  ...
  target.
- Three gates close the loops that work like this opens. `check-links.py`
  rejects a link to a path git ignores: such a target exists on a built
  machine and 404s for anyone who clones, which a plain existence check
  cannot see, and a directory of only ignored files (`outputs/`) is judged by
  its contents so `data/`, which mixes tracked scripts with a generated split,
  is left alone. The same tool reports a control character, the residue of a
  rewrite that masked a link and never restored it. `make lint` now runs
  `gen_agents_tree.py --check`, so a file added without `make agents-tree` no
  longer leaves a stale tree behind. And `tests/test_check_links.py`, fifteen
  cases, is the first test coverage the link checker has had.
- The generated `docs/results/` files are compared against a fresh render in
  `tests/test_reporting.py`. A hand edit there survives review and is then
  silently reverted by the next `make eval-report`; now the test names the
  file and the reason. The same file guards the changelog index: every
  `docs/changelogs/vX.Y.Z.md` must be linked from it, no link in it may
  dangle, and the version in `alire.toml` must have an entry, so a bump
  without a release log fails `make test`.
